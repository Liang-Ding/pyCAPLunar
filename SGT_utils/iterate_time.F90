...
!           if (height >= 3000.d0) then
  ...
  do it = it_begin,it_end
    ...
    if (.not. GPU_MODE) then
      ...
      ! Added by Liang Ding
      ! An example storing the interested area.
      if (myrank == 134 .or. myrank == 133 .or. myrank == 147 .or. myrank == 120 .or. myrank == 119 .or. myrank == 148) then
          if (SIMULATION_TYPE == 1 .and. SAVE_FORWARD) then 
             ! An example doing the temperal sub-sampling
             ! saving iterval = 50 * the_DT_in_Par_file.
             if (mod(it, 50) == 0) call dl_runtime_save_strain(it)
         endif
       endif
