# -------------------------------------------------------------------
# Tools to create synthetic waveforms.
...
def DSynRotateRTZ(mt, SGTs, ba_rad, n_lens):
    ...
    #  N-E-Z from SGT to RTZ in displacement.
    e = np.dot(SGTs[:, 1, :], mt)
    n = np.dot(SGTs[:, 0, :], mt)
...
def DSynENZ(mt, SGTs, n_lens):
    ...
    u_enz = np.zeros((3, n_lens))
    #  N-E-Z from SGT to RTZ in displacement.
    u_enz[0] = np.dot(SGTs[:, 1, :], mt)
    u_enz[1] = np.dot(SGTs[:, 0, :], mt)
    u_enz[2] = np.dot(SGTs[:, 2, :], mt)
    return u_enz
